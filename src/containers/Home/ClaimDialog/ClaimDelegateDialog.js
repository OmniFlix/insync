...
const ClaimDelegateDialog = (props) => {
    ...
    if (
      ...
      props.rewards.rewards.map((item) => {
        const tokens =
          ...
          item.reward.length &&
          item.reward.filter(
            (val) => val.amount > gasValue * config.GAS_PRICE_STEP_AVERAGE
          );
        if (tokens) {
          count += tokens.length;
        }
    ...
    if (
      ...
      props.rewards.rewards.map((item) => {
        ...
        if (
          tokens &&
          tokens >
            (gas.claim_reward + gas.delegate) * config.GAS_PRICE_STEP_AVERAGE
        ) {
          updatedTx.msgs.push(
            {
              typeUrl:
  ...
  if (
    ...
    props.rewards.rewards.map((value) => {
      ...
      rewards =
        rewards && rewards.amount && rewards.amount > gasValue
