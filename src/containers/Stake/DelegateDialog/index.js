...
const DelegateDialog = (props) => {
  ...
  const handleMultiDelegate = () => {
    ...
    if (
      ...
      props.selectedMultiValidatorArray.length > 1
    ) {
      gasValue =
        (gas.delegate * props.selectedMultiValidatorArray.length) / 1.1 +
        gas.delegate;
    }
        ...
        updatedTx.msgs.push({
          ...
          value: {
            ...
            amount: {
              amount: String(
                Math.floor(
                  (props.amount * 10 ** config.COIN_DECIMALS) /
  ...
  const disable =
    ...
    vestingTokens
      ? props.amount >
        parseFloat((available + vestingTokens) / 10 ** config.COIN_DECIMALS)
      : props.name === "Delegate" ||
        props.name === "Stake" ||
        props.name === "Multi-Delegate"
  ...
  return (
    ...
    >
      ...
      <DialogActions className="footer">
        ...
        >
          {inProgress ? variables[props.lang]["approval_pending"] : props.name}
        </Button>
      </DialogActions>
    </Dialog>
  );
