...
class NavBar extends Component {
  ...
  componentDidMount() {
    if (
      ...
      localStorage.getItem("of_co_wallet") === "cosmostation"
    ) {
      setTimeout(() => {
    ...
    if (
      ...
      !this.props.stake &&
      this.props.proposalDetails &&
      Object.keys(this.props.proposalDetails).length === 1 &&
      this.props.match &&
      this.props.match.params &&
      !this.props.match.params.proposalID
    ) {
    ...
    if (
      !this.props.inActiveValidatorsList.length &&
      !this.props.inActiveValidatorsInProgress &&
      !this.props.proposalTab
    ) {
    ...
    window.onload = () => {
      if (window.cosmostation && window.cosmostation.cosmos) {
        const cosmostationEvent = window.cosmostation.cosmos.on(
          "accountChanged",
          () => {
            if (
              localStorage.getItem("of_co_address") ||
  ...
  handleCosmoStation(fetch) {
    initializeCosmoStation((error, account) => {
      ...
      if (account && previousAddress !== account.address) {
        localStorage.setItem(
          "of_co_address",
          encode(account && account.address)
        );
