...
export const leapInit = (cb) => {
  (async () => {
    if (!window.leap.getOfflineSignerOnlyAmino || !window.leap) {
      const error = "Download the Leap Extension";
      cb(error);
    } else {
      if (window.experimentalSuggestChain) {
        try {
...
export const signTxAndBroadcastKeplr = (tx, address, cb) => {
  (async () => {
    (await window.keplr) && window.keplr.enable(chainId);
    const offlineSigner =
      window.getOfflineSignerOnlyAmino &&
...
export const signTxAndBroadcastLeap = (tx, address, cb) => {
  (async () => {
