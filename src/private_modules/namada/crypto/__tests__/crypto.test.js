import { AES, Argon2, Argon2Params, ByteSize, HDWallet, Mnemonic, PhraseSize, Rng, Salt, ShieldedHDWallet, } from "../crypto/crypto";
import { readStringPointer, readVecStringPointer, readVecU8Pointer, } from "../utils";
...
describe("Mnemonic", () => {
    test("It should return the correct number of words", () => {
        ...
        let words = readVecStringPointer(mnemonic === null || mnemonic === void 0 ? void 0 : mnemonic.to_words(), memory);
...
describe("HDWallet", () => {
    test("It should restore keys from a mnemonic and a path", () => {
        ...
        const defaultSecretKey = "116758879874284ffd7aad6af14d3ec24e2744671c667368698daca294799921";
        expect(defaultKey.to_bytes().length).toBe(KEY_LENGTH);
        expect(readStringPointer(defaultKey.to_hex(), memory)).toBe(defaultSecretKey);
        // Account 1: m/44'/877'/0'/0/1
        const path1 = new Uint32Array([44, coinType, 0, 0, 1]);
        const account1 = b.derive(path1);
        const expectedAccount1Pk = "fb348813623581154f240243b27ad2b5c7ecdc46a550526244cbea3e99c95a4a";
        expect(account1.to_bytes().length).toBe(KEY_LENGTH);
        expect(readStringPointer(account1.to_hex(), memory)).toBe(expectedAccount1Pk);
        // Account 2: m/44'/877'/0'/0/2
        const path2 = new Uint32Array([44, coinType, 0, 0, 2]);
        const account2 = b.derive(path2);
        const expectedAccount2Pk = "fda60501d0ba451b08d42f433aa3fb4223801a8d7296953273b20c3a2d24a7a2";
        expect(account2.to_bytes().length).toBe(KEY_LENGTH);
        expect(readStringPointer(account2.to_hex(), memory)).toBe(expectedAccount2Pk);
    });
});
